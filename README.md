...
> A Computer Vision university project focused on recognising visually similar vehicle classes under strong class imbalance.
...
- Designed a hierarchical **make -> model** prediction pipeline instead of treating all make-model pairs as one flat class space.
...
For make classification, the Focal Loss configuration increased the lowest per-class F1 from **0.3529** to **0.5000**, demonstrating a better outcome on underrepresented classes. See the complete methodology and analysis in [the project report](docs/Project_Report.pdf).
...
|- src/compcars/            # Models, datasets, metrics and utilities
|- Results/                 # Original experiment artefacts and metrics
...
| Path | Purpose |
| --- | --- |
| `scripts/make_main.py` | Make classification with Cross-Entropy. |
| `scripts/make_focal_main.py` | Make classification with Focal Loss and minority-class augmentation. |
| `scripts/fine_grained.py` | Hierarchical make-model classification. |
| `scripts/part_fine_grained.py` | Training/evaluation for one car-part classifier at a time. |
| `scripts/voting.py` | Accuracy-weighted ensemble of the eight part classifiers. |
| `src/compcars/` | Dataset loaders, ResNet models, loss functions, metrics and plotting utilities. |
| `docs/Project_Report.pdf` | Full technical report. |
...
```powershell
python -m scripts.make_main
python -m scripts.make_focal_main
python -m scripts.fine_grained
python -m scripts.part_fine_grained
python -m scripts.voting
```

The dataset, trained checkpoints and generated output are intentionally ignored by Git.

</details>

## Reference

Luca Fusaro, *Enhanced Fine-Grained Car Classification Using Focal Loss and Part-Based Voting Models*. See [the project report](docs/Project_Report.pdf) for the full methodology, dataset background and citations.
